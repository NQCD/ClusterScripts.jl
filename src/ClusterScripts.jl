...
TBW
"""
function loadbalance_queue(target_function, input_list::Vector{Dict{String, Any}}; trajectories_key="trajectories", ensemble_key="ensemble_algorithm", tmp_dir="tmp/", checkpoint_frequency=0, sort_variable="")
    ...
    # Multithreaded jobs requires modification of the input parameters to account for the trajectories%workers!=0 case. 
    ...
    # Find any input parameters where a reduction into chunks is sensible. 
    serial_jobs=findall(x-> x[ensemble_key]==EnsembleSerial() && x[trajectories_key]>1, input_list)
